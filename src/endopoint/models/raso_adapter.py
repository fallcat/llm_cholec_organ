"""RASO model adapter for endopoint."""
...
from .raso import load_raso_cholecseg8k, load_raso_cholec_organs, load_raso_cholec_gonogo


class RASOAdapter(ModelAdapter):
    """RASO model adapter for organ presence detection.
    
    Important: RASO only provides organ presence detection, not bounding boxes.
    All bbox fields will be null/empty when using RASO.
    """
    ...
    def __init__(self, 
        ...
        """Initialize RASO adapter.
        
        Args:
            model_name: Model identifier (e.g., "raso", "raso-cholecseg8k")
        ...
        self.cache_dir = Path(cache_dir) if cache_dir else Path("/shared_data0/weiqiuy/llm_cholec_organ/cache/raso")
        ...
        # Load the appropriate RASO model based on dataset or model name
        if self.verbose:
            print(f"Initializing RASO model adapter...")
        
        # Select model based on dataset or model name
        if dataset == "cholec_organs" or "organs" in model_name.lower():
            self.model = load_raso_cholec_organs()
        elif dataset == "cholec_gonogo" or "gonogo" in model_name.lower():
            self.model = load_raso_cholec_gonogo()
        else:
            # Default to CholecSeg8k
            self.model = load_raso_cholecseg8k()
        
        # RASO outputs labels exactly as they appear in the label files
        # No mapping needed - we'll keep the exact names
        
        # Classes that RASO typically doesn't detect (backgrounds)
    ...
    def _format_detection_response(self, detected_organs: List[str], requested_organs: List[str], 
        ...
        """Format RASO detection results as JSON response.
        
        Args:
            detected_organs: List of organs detected by RASO (lowercase names from RASO)
        ...
        """
        # RASO returns lowercase organ names
        ...
        for orig_name in original_organ_names:
            ...
            if organ_lower in self.non_detectable_classes:
                # Background is never detected by RASO
                is_present = False
            else:
            ...
            result[orig_name] = {
                "present": is_present,
                "bbox": None  # RASO doesn't provide bounding boxes
            }
    ...
    def _extract_organs_from_prompt(self, prompt: str) -> Tuple[List[str], List[str]]:
        ...
        Returns:
            ...
            - lowercase_names: List of organ names in lowercase for RASO
        """
    ...
    def __call__(self, prompts: Batch, *, system_prompt: str) -> Sequence[str]:
        """Process a batch of prompts through RASO.
        
        Args:
            prompts: Batch of queries, each a tuple of text/image parts
            system_prompt: System prompt (ignored for RASO)
        ...
        for query in prompts:
            ...
            if cached_response is not None:
                if self.verbose:
                    print(f"Using cached RASO response")
            ...
            if image is None:
                if self.verbose:
                    print("Warning: No image provided for RASO detection")
                response = json.dumps({"error": "No image provided"})
            else:
                try:
                    # Run RASO inference
                    output = self.model.analyze_image(image)
                    detected_organs = self.model.parse_output(output)
                    
                    if self.verbose:
                        print(f"RASO detected: {detected_organs}")
                    ...
                    if self.verbose:
                        ...
                        print(f"Lowercase names for RASO: {lowercase_names}")
                    ...
                    if self.verbose:
                        print(f"RASO response keys: {list(json.loads(response).keys())}")
                    
                except Exception as e:
                    if self.verbose:
                        print(f"Error in RASO inference: {e}")
