"""CholecGoNoGo dataset adapter for local files."""
...
# Class mappings for GoNoGo (matches the original notebook)
ID2LABEL: Dict[int, str] = {
    0: "Background",
    1: "Go (Safe to Incise)",
    2: "NoGo (Unsafe to Incise)"
}

LABEL2ID: Dict[str, int] = {v: k for k, v in ID2LABEL.items()}
LABEL_IDS: Sequence[int] = [1, 2]  # Excluding background
...
class CholecGoNoGoAdapter:
    """Dataset adapter for CholecGoNoGo dataset with Go/NoGo labels."""
    ...
    def __init__(
        ...
        self.gonogo_labels_dir = self.data_dir / gonogo_labels_dir
        
        if not self.images_dir.exists():
            raise ValueError(f"Images directory {self.images_dir} does not exist")
        if not self.gonogo_labels_dir.exists():
            raise ValueError(f"GoNoGo labels directory {self.gonogo_labels_dir} does not exist")
    ...
    def _index_dataset(self) -> None:
        ...
        for filename in tqdm(all_files, desc="  Checking files"):
            ...
            label_path = self.gonogo_labels_dir / filename
        ...
        print(f"CholecGoNoGo dataset indexed:")
    ...
    def example_to_tensors(self, example: Any) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # The label values are already 0-2 (background, go, nogo)
    ...
    def labels_to_presence_vector(
        ...
        Returns:
            LongTensor [K=2] with {0,1} (excluding background)
        """
    ...
    def get_bounding_boxes_with_labels(
        ...
        """Get bounding boxes with region labels.
...
@register_dataset("cholec_gonogo")
def build_cholec_gonogo(**cfg: Any) -> CholecGoNoGoAdapter:
    """Build CholecGoNoGo dataset adapter.
    
    Args:
        **cfg: Configuration parameters
        
    Returns:
        CholecGoNoGo adapter instance
    """
    return CholecGoNoGoAdapter(**cfg)