...
# Bounding Box Evaluation - Local Models - All 4 Scenarios
# This script runs all 4 scenarios for local models sequentially:
...
echo "=========================================="
echo "BOUNDING BOX EVALUATION - LOCAL MODELS - ALL SCENARIOS"
echo "=========================================="
echo
...
echo "⚠️  WARNING: Local models require significant GPU memory and time."
echo "    Each model will be loaded/unloaded for each scenario."
echo "    This process may take several hours for all scenarios."
echo
...
read -p "Continue with local model evaluation? (y/N): " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
...
for scenario in "${SCENARIOS[@]}"; do
    ...
    if ./eval_bbox_local_single_scenario.sh; then
        SCENARIO_END=$(date +%s)
    ...
    echo
    echo "Waiting 10 seconds to ensure GPU memory is cleared..."
    sleep 10
done
...
if [ ${#FAILED_SCENARIOS[@]} -eq 0 ]; then
    echo "✨ All local model scenarios completed successfully!"
    exit 0
else
