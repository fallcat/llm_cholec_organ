...
# Run ONLY zero-shot combined for CholecGoNoGo dataset
# This is faster than running all 4 scenarios

echo "Running Zero-shot Combined ONLY for CHOLEC_GONOGO dataset..."
echo
...
if [ "$MODEL_TYPE" == "api" ] || [ "$MODEL_TYPE" == "all" ]; then
    ...
    for model in "${API_MODELS[@]}"; do
        ...
        python3 eval_bbox_cholec_gonogo.py
...
echo
echo "==================== COMPLETE ===================="
echo "Zero-shot Combined evaluation complete for CholecGoNoGo!"
echo "Results saved in: results/bbox_cholec_gonogo_quick/zeroshot_combined/"
echo
echo "To view results, check the individual model directories or run:"
echo "  ls -la results/bbox_cholec_gonogo_quick/zeroshot_combined/"